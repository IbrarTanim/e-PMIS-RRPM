...
class MyProjectsSearchController extends GetxController
  ...
  void getSearchDataByName(bool isFromLoadMore) {
    ...
    APIRepository()
        .getProjectsListForMyProjectsSearchByName(nameValue, loadedPage).then((resp) {
      hideLoadingDialog();
  ...
  void getSearchDataByCode(bool isFromLoadMore) {
    ...
    APIRepository()
        .getProjectsListForMyProjectsSearchByCode(codeValue, loadedPage).then((resp) {
      hideLoadingDialog();
      if (resp.status == "success") {
        listResponse.value = ListResponse.fromJson(resp.data);
        var response = listResponse.value;
...
/*void getSearchDataByAgency(bool isFromLoadMore) {
  var agencyValue = searchController.text.trim();

    if (agencyValue.isEmpty || searchController.text.trim().isEmpty) {
      showToast("Search key can't be empty".tr, isError: true);
      return;
    }
    ...
    debugPrint("Rokan Name Value : $agencyValue" );
    APIRepository()
        .getProjectsListForMyProjectsSearchByAgency(agencyValue, loadedPage).then((resp) {
      hideLoadingDialog();
      if (resp.status == "success") {
        listResponse.value = ListResponse.fromJson(resp.data);
        var response = listResponse.value;
  ...
  /*void getSearchData(bool isFromLoadMore) {
    ...
    APIRepository()
        .getProjectsListForMyProjectsSearch(searchKey, loadedPage, searchType)
        .then((resp) {
      hideLoadingDialog();
      if (resp.status == "success") {
        // loadedPage++;
