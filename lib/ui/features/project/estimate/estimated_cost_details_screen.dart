...
import 'package:pmis_flutter/ui/features/project/estimate/estimate_cost_details_list_item.dart';
...
class NewProjectEstimatedCostDetailsScreen extends StatefulWidget {
  final AllProjectsResponse? allProjectsResponse;
  final bool isMyProject = false;

  const NewProjectEstimatedCostDetailsScreen(
      {Key? key, this.allProjectsResponse})
      : super(key: key);

  @override
  _NewProjectEstimatedCostDetailsScreenState createState() =>
      _NewProjectEstimatedCostDetailsScreenState();
}

class _NewProjectEstimatedCostDetailsScreenState
    extends State<NewProjectEstimatedCostDetailsScreen> {
  final _controllerForProjectDetailsByID = Get.put(ProjectDetailsByIDController());
      ...
      body: SafeArea(
        child: Obx(() {
          return ListView.separated(
            ...
            itemCount: _wishListProjectDetailsController.projectEstimatedCostResponse.length,
            separatorBuilder: (context, index) => const SizedBox(height: 6),
            itemBuilder: (context, index) {
              return NewProjectEstimatedCostDetailsItemView(
                projectEstimatedCostResponse: _wishListProjectDetailsController.projectEstimatedCostResponse[index],
              );
            },
          );
        }),
