...
class AgencyWiseController extends GetxController {
  ...
  void getMinistryDropDownList() {
    showLoadingDialog();
    APIRepository().getMyProjectsMinistryDropDownList().then((resp) {
      hideLoadingDialog();
  ...
  void getDivisionDropDownList() {
    APIRepository()
        .getMyProjectsDivisionDropDownList(
            selectedValueForMinistry.value.value ?? "")
        .then((resp) {
      hideLoadingDialog();
      if (resp.status == "success") {
        if (resp.data != null) {
          DivisionResponse divisionResponse =
              DivisionResponse.fromJson(resp.data);
          if (divisionResponse.divisions != null &&
              divisionResponse.divisions!.isNotEmpty) {
            ...
            selectedValueForDivision.value = ValueObject(value: "");
            if (divisionResponse.agencies != null &&
                divisionResponse.agencies!.isNotEmpty) {
  ...
  void getAgencyDropDownList() {
    ...
    APIRepository()
        .getMyProjectsAgencyDropDownList(
            selectedValueForDivision.value.value ?? "")
        .then((resp) {
      hideLoadingDialog();
      if (resp.status == "success") {
        if (resp.data != null) {
          List<ValueObject> list = List<ValueObject>.from(
              resp.data!.map((x) => ValueObject.fromJson(x)));
          // ministryListResponse.addAll(list);
  ...
  /// *** Ministry List Data ***///
  void getAllAgencyListFromMyProjects(bool isFromLoadMore) {
    if (!isFromLoadMore) {
      loadedPage = 0;
      hasMoreData = true;
      allProjectsResponseList.clear();
