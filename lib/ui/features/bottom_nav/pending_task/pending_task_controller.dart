...
class PendingTaskController extends GetxController {
  ...
  RxList<PendingTaskResponse> allProjectsResponseList = <PendingTaskResponse>[].obs;
  ...
  void getPendingTaskList(bool isFromLoadMore, String? userId) {
    if (!isFromLoadMore) {
      loadedPage = 0;
      hasMoreData = true;
      allProjectsResponseList.clear();
    ...
    APIRepository().getPendingTaskProjectsList(loadedPage, userId ?? "").then((resp) {
      isLoading = false;
      hideLoadingDialog();
      if (resp.status == "success") {
        ...
        if (response.lists != null  && response.lists!.isNotEmpty) {
          List<PendingTaskResponse> list = List<PendingTaskResponse>.from(response.lists!.map((x) => PendingTaskResponse.fromJson(x)));
